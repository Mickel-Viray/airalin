...
export default async function ReviewerPage({
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 space-y-6 pb-16 print:p-0 print:m-0 print:max-w-none">
      ...
      <div className="border-b pb-4 space-y-1 print:border-none">
        ...
        <h1 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 pt-1 break-words">
      ...
      <Tabs defaultValue="guide" className="w-full mt-6">
        ...
        <TabsContent value="guide" className="focus-visible:outline-none focus-visible:ring-0 mt-0">
          <div className="w-full max-w-full bg-white p-4 sm:p-8 md:p-12 rounded-2xl border border-slate-200 shadow-sm break-words overflow-hidden print:border-none print:shadow-none print:p-0">
            <ReactMarkdown
              remarkPlugins={[remarkGfm]}
              components={{
                h1: ({ node, ...props }) => (
                  <h1 className="text-xl sm:text-2xl font-bold mt-8 mb-4 text-slate-900 break-words" {...props} />
                ),
                h2: ({ node, ...props }) => (
                  <h2 className="text-lg sm:text-xl font-bold mt-8 mb-4 text-slate-900 border-b pb-2 break-words" {...props} />
                ),
                h3: ({ node, ...props }) => (
                  <h3 className="text-base sm:text-lg font-bold mt-6 mb-3 text-slate-900 break-words" {...props} />
                ),
                p: ({ node, ...props }) => (
                  <p className="mb-4 leading-relaxed text-slate-700 break-words" {...props} />
                ),
                ul: ({ node, ...props }) => (
                  <ul className="list-disc pl-5 sm:pl-6 mb-4 space-y-2 text-slate-700 break-words" {...props} />
                ),
                ol: ({ node, ...props }) => (
                  <ol className="list-decimal pl-5 sm:pl-6 mb-4 space-y-2 text-slate-700 break-words" {...props} />
                ),
                strong: ({ node, ...props }) => (
                  <strong className="font-semibold text-slate-900" {...props} />
                ),
                code: ({ node, className, ...props }) => (
                  <code className="bg-slate-100 text-slate-800 text-xs px-1.5 py-0.5 rounded font-mono break-all" {...props} />
                ),
                pre: ({ node, ...props }) => (
                  <div className="overflow-x-auto max-w-full my-4 rounded-xl bg-slate-900 p-4 text-slate-100 text-xs">
                    <pre {...props} />
                  </div>
                ),
                table: ({ node, ...props }) => (
                  <div className="w-full overflow-x-auto my-6 rounded-lg border border-slate-200">
                    <table className="w-full text-left border-collapse min-w-[500px]" {...props} />
                  </div>
                ),
                th: ({ node, ...props }) => (
                  <th className="border-b-2 border-slate-200 bg-slate-50 py-2.5 px-3 sm:py-3 sm:px-4 font-semibold text-slate-900 text-xs sm:text-sm" {...props} />
                ),
                td: ({ node, ...props }) => (
                  <td className="border-b border-slate-100 py-2.5 px-3 sm:py-3 sm:px-4 text-slate-700 text-xs sm:text-sm" {...props} />
                ),
              }}
            >
